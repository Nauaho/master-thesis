    ...
    def import_data(self):
        try:
            ...
            with self._conn.cursor() as cur:
                with cur.copy(
                    "COPY subreddits (name, embedding) FROM STDIN (FORMAT BINARY)"
                ) as copy:
        ...
        count = self._conn.execute(
            "SELECT count(*) FROM subreddits WHERE embedding IS NOT NULL"
        ).fetchone()[0]
        ...
        self.optimal_ivf_probes_number = math.floor(
            math.sqrt(self.optimal_ivf_lists_number)
        )
